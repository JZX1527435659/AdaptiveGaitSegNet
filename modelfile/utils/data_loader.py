...
from .data_set import DataSet  # 假设你有一个定义好的 DataSet 类
...
def load_data(dataset_path, resolution, dataset, pid_num, pid_shuffle, cache=True):
    ...
    for _label in sorted(os.listdir(dataset_path)):
        # In CASIA-B, data of subject #5 is incomplete. Skip it.
    ...
    # Save/load partition information
    ...
    if not osp.exists(pid_fname):
        ...
        # Save using dtype=object to avoid ValueError from inhomogeneous shape
        np.save(pid_fname, np.array(pid_list, dtype=object))
        print(f"[Partition] Created partition file: {pid_fname}")
    else:
        print(f"[Partition] Loaded partition file: {pid_fname}")

    pid_list = np.load(pid_fname, allow_pickle=True)  # allow_pickle=True for dtype=object
