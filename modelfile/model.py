...
class Model:
    def __init__(self,
                 ...
                 test_source,
                 img_size=64,
                 ce_weight=0.5,  # 添加交叉熵损失权重
                 triplet_weight=0.5):  # 添加三元组损失权重
        ...
        # 初始化 SetNet 并传入 num_classes
        self.encoder = SetNet(self.hidden_dim, num_classes=train_pid_num).float()  # 传入 num_classes
        ...
        self.optimizer = optim.Adam(
            [{'params': self.encoder.parameters()},
             {'params': self.triplet_loss.parameters()}],  # 添加 triplet_loss 的参数
            lr=self.lr)
    ...
    def load(self, restore_iter):
        ...
        # 统一把 checkpoint 中的 "module." 剥掉
        new_state_dict = {k[7:] if k.startswith('module.') else k: v
                          for k, v in state_dict.items()}

        # 如果当前 encoder 是 DataParallel，再加回一层 "module."
        ...
        self.encoder.load_state_dict(final_state_dict, strict=False)  # 允许缺省