...
from .data_loader_2 import load_data  # 使用适配后的数据加载
from .model_2 import Model


def initialize_data(config, train=False, test=False):
    print("Initializing data source (GAIT-IST adapted)...")
...
def initialize_model(config, train_source, test_source):
    ...
    model_param['train_pid_num'] = 2  # 二分类

    batch_size = int(np.prod(model_config['batch_size']))
    model_param['save_name'] = '_'.join(map(str, [
        model_config['model_name'],
        data_config['dataset'],
        data_config.get('data_type', 'silhouettes'),
        data_config.get('direction', 'front'),
        batch_size,
        model_config['frame_num'],
    ]))

    # 添加早停参数
    model_param['early_stop_patience'] = model_config.get('early_stop_patience', 100)
    model_param['early_stop_delta'] = model_config.get('early_stop_delta', 0.001)
...
def initialization(config, train=False, test=False):
    print("🔧 Initializing (GAIT-IST adapted)...")
